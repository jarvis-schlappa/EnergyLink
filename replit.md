...
- **E3DC Modbus TCP Integration (HARDWARE-VALIDIERT):** Backend-Integration mit modbus-serial Library vollständig implementiert und mit E3DC S10 Hardware erfolgreich getestet. Neuer API-Endpoint `/api/e3dc/live-data` liest E3DC S10 Register über Modbus TCP (Port 502): PV-Leistung, Batterie-Leistung/SOC, Hausverbrauch, Netzleistung, Autarkie, Eigenverbrauch. Wallbox-Leistung wird zusätzlich via KEBA UDP Report 3 abgefragt. Register-Mapping korrigiert: Little-Endian (LSW first) für INT32-Werte, korrekte Offsets 67-83. Frontend zeigt Live-Daten mit 5-Sekunden Auto-Refresh. Connection Recovery: Bei Modbus-Fehlern wird Verbindung automatisch geschlossen und beim nächsten Request neu aufgebaut. E3DC IP-Adresse in Settings konfigurierbar. Debug-Logs kompakt in einer Zeile bei LogLevel DEBUG.
- **E3DC-Seite Redesign:** Energieflussdiagramm entfernt. Neue Struktur analog zur Wallbox-Seite mit großer Status-Card (Batterie SOC + Leistung als Hauptinfo), kleineren Info-Cards für PV/Wallbox, und Effizienz-Card. Dezente Farben (text-muted-foreground) statt bunter Icons. Leistungswerte unter 10 kW in Watt angezeigt, ab 10 kW in kW.
